// Shared shim harness for the runAssertCompletion suites.
//
// Extracted from gc-assert-completion.runassertcompletion-thin-completion.test.js
// when issue #1679's delivery-binding regressions pushed that file past the
// 500-line limit (docs/CODING_STANDARDS.md). The harness is unchanged.
...
export async function withShimPath(binDir, fn) {
...
export function makeCompletionShimRepo({
  comments = [],
  ...
  // Issue #1679 (core-F1): the branch the trusted review publication ran on.
  // Defaults to the PR's own head so the happy path stays the happy path.
  reviewedBranch = null,
  // Issue #1679 (core-F2): the head the synchronization record binds to.
  // Defaults to the pull request's own head, so the happy path stays green.
  synchronizedHead = "a".repeat(40),
  // The lane the synchronization record was written for. A quickfix record names
  // no review publication ("-"), exactly as the sync boundary writes one.
  syncLane = "implement",
} = {}) {
  const ISSUE = 1103;
  ...
  comments = [...comments,
    ...
    { id: 8998, user: { login: "fake" }, author_association: "OWNER", body: `<!-- gc:codex-prepush-cycle issue="1103" branch="${reviewedBranch ?? `${ISSUE}-branch`}" cycle="1" ${provenance} -->\n\n_gc_codex_review pre-push cycle 1 complete` },
    ...
    { id: 8996, user: { login: "fake" }, author_association: "OWNER", body: `<!-- gc:implement-base-sync schema="gc.implement.remote-base-sync/v2" record="${"4".repeat(32)}" issue="1103" branch="1103-branch" base="dev" source="refs/remotes/origin/dev" pre="${"e".repeat(40)}" fetched="${"f".repeat(40)}" outcome="merged_clean" result="${synchronizedHead}" verified="${"5".repeat(40)}" settled="${"1".repeat(40)}" review="${syncLane === "quickfix" ? "-" : "a".repeat(64)}" revision="${syncLane === "quickfix" ? "-" : "c".repeat(64)}" lane="${syncLane}" -->` },
...
export function makeFailShimRepo() {
