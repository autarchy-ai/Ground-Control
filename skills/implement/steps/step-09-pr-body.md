...
1. **Render the PR body via `gc_render_pr_body`** (per ADR-036). Pass:
   ...
   - `lane` / `pre_push_reviews` (optional): leave both unset. They default to `implement` / `completed`, which is the accurate attestation for this lane - Step 6.5 runs the pre-push Codex review before this step. `/implement` may not pass `pre_push_reviews: "not_run"`; the renderer refuses that combination rather than let a mandatory gate be attested away (issue #1551).
