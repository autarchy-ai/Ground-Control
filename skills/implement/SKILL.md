...
- **Step 6.5 at the cap**: fix and verify every known finding, then present the user with the decision to spend one additional Codex cycle or proceed. An explicit instruction to proceed without another cycle (including “proceed without”, “continue”, or a decline) is a decision to accept the cap: set `accepted_at_cap` and advance to Phase C immediately. Do not re-ask for review authorization, frame this choice as a blocker, or imply that another review is required. The cap limits reviewer iterations and does not require a clean terminal verdict. Record only genuinely unresolved findings as execution obligations. The label set in Step 1 stays until the issue is closed at Phase E.
...
- **Issue close mechanism (issues #1058 + #1156 + #1541)**: the GitHub issue is closed at **Phase E (Step 20)** by `gc_close_issue_after_merge`, which verifies the linked PR is merged (`merged_at` non-null AND state `MERGED`) and — for an OPEN issue — requires a trusted `gc:final-report` marker (proof that Phase E's merged-requirement-state validation succeeded) before closing, refusing with `close_requirement_state_unverified` otherwise (`override` + `override_reason` is the recorded escape hatch). For a **requirement-backed** run the PR body uses a non-closing `Refs #<issue-number>` (Step 9), so GitHub does NOT auto-close at merge — the validated close tool is the sole closer, and the issue stays open until Phase E's report is posted. For a **requirement-free** run the body keeps `Closes #<issue-number>`, which GitHub honors only when the PR merges into the repository's default branch: there Step 20 reaches its idempotent `already_closed` no-op, and on a PR merged into the integration branch Step 20 closes the still-open issue through the same marker gate (issue #1601). The `in-progress` label is operational-only. `gc_close_issue_after_merge` drops it at the shared close boundary once the issue is actually closed, best-effort, so a failed removal never changes the close outcome (#1686); no agent step removes it, and an issue left open by a refused finalization keeps it, because the work is still open. Phase E is invoked by re-running `/implement <issue>` after the merge; Step 1's orchestrator detects the Phase-D-complete state - the `ready_for_review` marker present + a merged linked PR + Phase E not yet recorded (no `gc:final-report` marker) - and short-circuits to **Step 17 `phase="post_merge"`** (verify merged requirement state → report → close), not back into Phase A–D. The detection keys on the `gc:final-report` marker being absent, NOT on the issue being open. If the PR exists but is not yet merged, Phase D is complete and the run STOPS awaiting the user's merge; it does not redo Phase A–D. `gc_close_issue_after_merge` performs only linked-PR resolution, merge-state verification, the requirement-state marker gate, and the idempotent close (ADR-089); it does not list open issues, rank candidates, or return a next-issue recommendation.
- **Automated Phase E (issue #1671, ADR-102)**: Phase D's `readiness` action records a trusted **delivery handoff**: an issue-thread record carrying the exact tool-shaped completion payload, digest-bound to the issue, the PR, and the PR head OID whose required hosted checks it just verified, plus a pointer comment on the PR naming that record. `.github/workflows/ground-control-phase-e.yml` runs on the merged PR and replays that payload through the same `gc_implement_mechanical action="finalize"`, so **the agent may terminate permanently once Phase D is recorded** and the merge alone finishes the workflow. Nothing in this skill polls for or waits on the merge. A merged PR with no delivery pointer is a successful no-op; a readiness record bound to a different head than the one that merged is refused, so a post-readiness push means readiness must be re-recorded before merge. Replay is safe, because post-merge publication treats an existing trusted final-report marker for this issue and PR as success rather than posting a second report. A failed finalization leaves one bounded `gc:delivery-finalization-failed` record, leaves the issue OPEN, and fails the job; repair the cause and re-run the workflow for that PR, or call `gc_finalize_merged_pr`. `readiness` is lane-discriminated: `lane="quickfix"` records the same neutral handoff and still gets no pre-merge report and none of /implement's requirement or review gates.
- **Single human touchpoint**: PR merge (between Phase D and Phase E). The orchestrator never runs `gh pr merge`. Phase E runs autonomously: normally in the merged-PR workflow with no model or agent session, and otherwise when the user re-invokes `/implement` after the merge. There is no synchronous user gate inside Phase E.
