...
flowchart TB
  ...
  S8b[8.5 · Pre-push gc_codex_review · core + security · default cap 1 · retain locally, then publish sanitized record]
  S9[9 · Publish · stage + configured pre-commit boundary + commit + push + base sync]
  S10[10 · Create PR to dev · Refs #issue when requirement-backed]
  S11[11 · CI monitor]
  S12[12 · SonarCloud sweep]
  S17r[17 · Pre-merge readiness record + trusted delivery handoff · proposed state · DO NOT MERGE]
  End([User reviews PR and merges · agent may terminate here])
  S17d[17 · Phase E · merged-PR workflow · verify merged requirement state at merge revision + final report]
  ...
  S8 -->|fail| S6
  S9 -->|hook fails · repair · new publish attempt| S9
...
- **Step 6** is TDD (red → green → refactor per clause) under four explicit paths: **A**, new requirement/feature (test the missing behavior first); **B**, shipped-code bug fix (reproduce the reported defect on the unmodified buggy tree before repair); **C**, reviewer-finding fix (lock executable or runtime-data repairs with regression evidence in the same review cycle); and **D**, prose-only/static contract narrowing (the existing documentation-only carve-out may apply). Step 1's feature/bug-fix/mixed `implementation_intent` is informational; the Step 4 plan assigns the authoritative `tdd_path` per clause, so mixed issues use multiple paths. Runtime configuration, schemas, grammars, fixtures, policy data, and executable renames cannot use the documentation-only carve-out. Step 8 is targeted local verification, and Step 9's `publish` action runs the single pre-commit invocation. The carve-out lives in `skills/implement/steps/step-04.4-tdd.md` and remains limited to diffs with no executable behavior whose claims are protected by an existing structural gate (policy check, schema validator, lint rule, verifier script). It must be declared in the plan and re-stated as an issue comment naming the gate; substring/snapshot tests written only to satisfy TDD wording are explicitly disallowed. The pre-publish re-validation checks it with a two-check sweep over the union of committed, staged, unstaged, and untracked paths (Step 6 runs before stage-and-commit, so working-tree state is part of the diff): every path must be in the documentation set AND every diff hunk's content must be free of executable behavior; a path check alone isn't enough, because a doc file can still carry executable behavior.
- **Local verification (step 8) is targeted, not repository-wide.** The agent runs the narrowest tests that exercise the changed behavior, and the `publish` action runs `cfg.workflow.precommit_command` once before it commits. No local step runs `cfg.workflow.completion_command` or `cfg.workflow.policy_command` (`make mcp-test` and `make policy` here): CI owns both suites and blocks the PR on either failing (development principle 8, issue #1694). The server-side project quality-gate evaluation that once ran here (`gc_assert_quality_gates`, issue #1101) was retired with the backend it queried, and requirement traceability is verified against the merged files at Step 17.
- **Step 8.5 (= SKILL Step 6.5)** is the pre-push Codex review pass per issue #804. `gc_codex_review_cycle` runs with `uncommitted=true` and `publication_mode="deferred"` against the complete tracked candidate (committed, staged, and unstaged changes) diffed from its merge base with the requested base branch, so a resolved but uncommitted merge reviews only the feature work (issue #1694), retaining the original result and exact revision in protected per-worktree Git metadata without a GitHub write. The agent inspects the opaque handle, prepares a complete sanitized one-to-one rendering, and calls `gc_publish_review_result` before changing the tree. Publication rechecks the revision and cycle slot, then writes provenance-bound findings, cycle, and decision records idempotently; only that publication consumes a cycle. Review execution and publication are observational and never satisfy or block readiness, synchronization, PR creation, or completion. **Default cap is 1 cycle** (issue #906); configurable per repo via `workflow.codex_review.pre_push_cap` in `.ground-control.yaml`, bounds `[1, 10]`. The cap is enforced **per issue**. After the last in-cap cycle, the agent fixes or explicitly dispositions every finding, runs proportionate verification, and asks whether to run one additional cycle or proceed. If the user declines the extra cycle, the review resolves as `accepted_at_cap` and advances to Phase C; a clean terminal verdict is not required. `override_cap=true` plus a quoted `override_reason` authorizes only the extra cycle. No commit or push occurs between cycles. See ADR-099.
- **Fix-locks-itself evidence (Step 6.5).** The canonical rule in `skills/implement/steps/_review-loop-rules.md` requires each accepted fix to executable code or a runtime-consumed data contract to add or extend a test that fails when the named defect is reintroduced, with self-verification naming the test path and case. Pure prose may state that there is no executable surface to lock.
- **Automated cap disposition (optional, default off; issue #1245).** When `workflow.review_disposition.enabled` is true, the Codex cap boundary can be dispositioned automatically after the last-in-cap findings are fixed, self-verified, and re-staged. `proceed` advances with the same `accepted_at_cap` meaning; `one_more_cycle` consumes one marker-authorized over-cap cycle; `escalate_to_human` asks the same binary extra-cycle-or-proceed question. The hard `max_auto_overrides` ceiling remains one by default.
- **Deterministic execution bands (#1426/#1473/#1637).** Successful-path mechanical work is composed by `gc_implement_mechanical` instead of consuming a separate model turn per step: `bootstrap` gathers issue context and pickup state, `publish` owns staging through synchronization, `monitor` observes CI and Sonar together, `readiness` records `/implement` pre-merge status, and `finalize` performs the post-merge record and close. `/quickfix` is a lane-discriminated thin caller of `bootstrap`, `publish`, `monitor`, and `finalize`; its bootstrap refuses requirement-backed issues before branch mutation, and its finalizer accepts the slim requirement-free completion shape. The canonical workflows call the two long actions (`publish`, `monitor`) with `async: true` and one bounded `idempotency_key` per logical attempt, then await the job through `gc_codex_job`; short actions remain synchronous. The same key and normalized input reuse one running or terminal job, different input under the key is refused, and distinct active `publish` attempts cannot race on one checkout. A terminal job preserves the action envelope under `result`: red tests, hooks, merge conflicts, CI, or Sonar are completed jobs with `result.ok: false` and bounded repair evidence, not transport failures. After repair the caller uses a new key. Publish conflicts retain the exact synchronization evidence required to resume the preserved merge. Mechanical jobs return `job_not_cancellable` because their full subprocess and polling graph does not yet honor abort; the `publish` hang this closes is prevented instead by the shared gate runner reaping its process tree on the leader's exit (a leaked descendant can no longer hold the stdout pipe and keep the runner running after every visible child has exited), a per-worktree mutation lease, a versioned write-ahead recovery journal, a pre-commit compare-and-swap on `HEAD`/`MERGE_HEAD`, and restart-time reconciliation that resumes a journal-matching merge through the base-sync retry contract or refuses ambiguous state without mutating (issue #1495, ADR-036). Architecture, implementation, review finding decisions, and post-merge traceability reconciliation remain agent work.
- **Requirement identity for repository gates (#1434).** A repository whose completion, policy, or pre-commit command runs a requirement-governance check normally derives the requirement under test from the branch name. `/implement` can target a requirement whose issue branch carries no UID, so an optional `requested_requirement_uid` on `gc_implement_mechanical` and `gc_synchronize_implement_branch` reaches every repo-authored gate as the `ACES_REQUIREMENT_UID` environment variable: the `publish` pre-commit command. The value travels in the child environment rather than the command text, so it never enters argv and offers no interpolation point. A well-formed UID is not authority: every action that can reach a gate resolves the requested UID server-side against the target issue's canonical Requirements section and refuses an unlisted one before any gate runs. Each of these actions is independently callable, so `bootstrap`'s membership check protects only its own entry point; without the shared binding a caller could name a requirement from another issue or project and have the repository's governance gate evaluated, and attested, against it. Omitting the input changes nothing: no variable is injected, and branch-derived governance behaves exactly as before. The environment is the only place the value exists; it is never added to result envelopes, synchronization markers, or issue comments.
- **Steps 7–11** stage, commit, push, synchronize the remote integration branch, open the PR, and block on CI + SonarCloud. **Step 8.5 pre-PR synchronization (#1421):** `gc_synchronize_implement_branch` fetches the configured base into `refs/remotes/origin/<base>` with an explicit refspec and either records `already_current` or leaves a real `--no-ff --no-commit` merge for conflict resolution in the invocation checkout. It verifies the merge graph, pushes normally, and posts a trusted versioned issue-thread attestation containing the fetched-base and resulting-feature SHAs. Step 9 renders the body, then `gc_create_synchronized_implement_pr` re-fetches the base and refuses the GitHub write unless that attestation, the local head, and the remote feature head still match. The canonical workflow has no direct CLI PR-creation fallback. **PR title format (issue #901):** Step 9 validates the title locally and again at the MCP creation boundary. The title uses one conventional-commit type with optional scope, an optional breaking-change `!` before the colon (`feat!:`, `feat(api)!:`; issue #1593), and a lowercase-leading subject; per-repo `workflow.pr_title` overrides remain authoritative.
- **CI-owned broad verification (#1629).** The mechanical `verify` action is removed. Implementation and review repairs use targeted local tests. Publish and base synchronization run no completion or policy suites; synchronization still checks the merge graph and published tree. The local verification-attestation, fingerprint, and phase-cache machinery is retired. Required hosted checks for the current PR head are read directly at readiness; missing, pending, failed, or unavailable evidence blocks it.
- **Progressive remote remediation (#1628).** CI and Sonar are observed concurrently. An actionable failure returns immediately with its `head_sha`, remaining `monitor_jobs`, and `resume` input. Poll child handles while batching fixes. A new head starts new monitoring; old diagnostics cannot clear readiness. Timing fields report time to actionable evidence and waiting after detection.
...
- **Steps 6.6, 13, and 14 are tombstones.** ADR-099 removed the test-quality review stage and its MCP tools entirely. Final CI re-verify (former Step 14) collapsed into Step 10's existing CI watch. **Steps 18 / 19 were consolidated into Step 17 by issue #1103.**
...
- **Automated Phase E (issue #1671, ADR-102).** Phase D's `readiness` action writes a trusted **delivery handoff** as well as the readiness record: an issue-thread record carrying the exact tool-shaped completion payload, digest-bound to the issue, the PR, and the PR head OID whose required hosted checks it just verified, plus a pointer comment on the PR naming that record. PR title, body, labels, branch name, and closing keywords confer no authority. `.github/workflows/ground-control-phase-e.yml` then runs on a merged pull request and replays that payload through the unchanged `gc_implement_mechanical action="finalize"`, so an agent may terminate permanently at a ready PR and the merge alone finishes the workflow. The job is a trigger and a transport: no `gh` logic, no marker parser, no completion reconstruction, no tests, no policy suite, no review, and no waiting on any other post-merge job. It never uses `pull_request_target` and never checks out the PR head; it pins the event's immutable `merge_commit_sha`, sets `persist-credentials: false`, and holds `issues: write` as its only write permission. `tools/policy/phase_e_automation.py` pins that shape two-sidedly, because the workflow's path is also the close gate's trust anchor.

  A merged PR with no delivery pointer is a successful no-op, because most merged PRs are not Ground Control deliveries. A readiness record bound to a different head than the one that merged is refused, so a post-readiness push means readiness must be re-recorded before merge. Replay is safe: post-merge publication now treats an existing trusted final-report marker for the same issue and PR as success rather than posting a second report, and the close was already idempotent. A failed finalization writes one bounded, scrubbed, idempotently keyed `gc:delivery-finalization-failed` record naming the PR, the readiness record, a stable error code, and the repair. It then leaves the issue **open** and fails the job. Repair the cause and re-run the workflow for that PR, or call `gc_finalize_merged_pr`. Re-invoking `/implement <issue>` after a merge still works and is the fallback.

  Automation authorship is verified, not asserted. `gc_close_issue_after_merge` accepts a final-report marker from the repository's own Actions identity, which the collaborator endpoint reports as `permission: "none"`, only when the separate `gc:finalizer-run` marker beside it names a run that resolves, through the Actions API, to this repository's pinned finalizer workflow and is bound to this PR (or is a `workflow_dispatch` run, which only a repo-write user can start). That class unlocks nothing else: execution-obligation `wontfix` authorization, the merged-state override, and the delivery-readiness record itself still require a repo-write human. A workflow only processes events once it exists on the branch the event resolves its definition from, so the first delivery after the automation lands may still need one manual finalizer run.

- **Waiting costs requests, not just turns (issue #1671, ADR-102).** Issue #1669 removed the per-tick model turn from waiting; the GitHub half remained. The monitor loop re-read the remote-gate snapshot every 15 seconds, and that snapshot is five REST calls, so one watched head could spend several hundred requests against a 5,000/hour token shared by every agent and tool on the host. Those four endpoints are now revalidated with `If-None-Match`: GitHub answers `304 Not Modified` when nothing changed, and a 304 does not count against the primary rate limit. An all-unchanged probe reuses the previous snapshot, reports `unchanged: true`, and the loop backs its cadence off from 15 seconds toward 2 minutes while the head stays quiet, resetting the moment anything moves. A probe that errors, or a response that declared a next page, is never read as stability. The CI run watcher keeps its fixed cadence on purpose: its queued and timeout reasoning is built on regular sampling, and a longer interval can step over a short-lived state such as #1581's between-jobs `queued` gap.

- **A conflicted PR is reported, not waited out (issue #1671).** When a delivery PR conflicts with its base, GitHub never builds the merge ref, so its checks never run. The monitor reads the PR's merge state and refuses on `DIRTY` with `monitor_pr_conflicted` and `next_action: resolve_the_base_conflict_then_rerun_publish_and_monitor` - before starting a watcher, and again on each tick, because the base can move under an open PR. Previously this polled out the full 45-minute cap and returned a timeout that named nothing actionable.

- **`/quickfix` records the same handoff (ADR-100 amendment).** `readiness` is lane-discriminated rather than implement-only: `lane="quickfix"` reads the same hosted-check bar for the current head and records the delivery handoff, with no pre-merge report and none of /implement's requirement or review gates. Its slim outcome is still written by the shared finalizer at Q7.
...
- **Every downstream failure loops back to step 9** (stage + commit + push), which is the single re-entry point for fix commits. Targeted local verification (step 8), the pre-push codex review (step 8.5), and the GC verify (step 17) are the loops that target earlier steps, because they correspond to local-only / pre-PR / GC-only state respectively.
...
| Durable-record MCP tools | `gc_get_review_result` and `gc_publish_review_result` separate local review inspection from provenance-bound sanitized publication; `gc_post_decision_record` renders cycle decisions, `gc_post_final_report` renders the report substep invoked through the shared finalizer, and `gc_render_pr_body` renders the Step 9 PR body. `gc_render_pr_body`'s evidence envelope is repo-neutral (#1199): semantic, stack-agnostic attestations, never configured command strings. Publication tools filter sensitive content, post under structured marker families, and reject `decision: "defer"` server-side. `gc_post_final_report` also requires `/implement` callers to pass `plain_english_outcome`, which renders an Outcome section before the structured evidence. `gc_render_pr_body` takes `lane` and `pre_push_reviews` (issue #1551) to decide which pre-push Codex review attestation the Ground Control Checks section carries; `/quickfix` records whether its one optional Codex cycle ran. Every body carries an accurate attestation. | Always available; shared composites call the mutation tools at their owned boundaries |
| Merged requirement-state + post-merge close gates (#1058/#1156/#1103/#1541/#1637) | `gc_assert_completion` (Step 17). `phase="post_merge"` is merge-gated, then re-derives in-scope UIDs from the issue and verifies every requirement at the linked PR's immutable merge revision (UID path, frontmatter id, expected status, required traceability), refusing (`completion_requirement_state_unverified` / `completion_scope_mismatch`) before `gc_post_final_report` on any mismatch and rendering observed merged values; `override` + `override_reason` is the recorded escape hatch. The requirement transition and traceability edits are made pre-publish in the delivery diff (issue #1541). `gc_close_issue_after_merge` verifies the linked PR's `merged_at` nonnull and state `MERGED`, and - for an open issue - a trusted `gc:final-report` marker before closing; it is idempotent on already-closed issues. Requirement-backed runs use a non-closing `Refs #<n>`; requirement-free runs keep `Closes #<n>`, which closes the issue only on a default-branch merge. `/quickfix` Q7 reaches the same report and close gates through `gc_implement_mechanical action="finalize"`; it remains requirement-free and omits implement-only merged-requirement reconciliation and outcome fields. | Always on for `/implement`; `lane: "quickfix"` selects the slim shared finalizer |

Each new tool is deterministic and structured-input/output, with no LLM call in the tool itself.

## Review Pipeline

One mandatory pre-implementation architecture pass, then a single bounded pre-push Codex review pass (Step 6.5). The separate test-quality reviewer was removed by ADR-099. The post-push Codex review (former Step 12) was removed by issue #804. Merge-commit drift relative to base is the responsibility of CI and SonarCloud, not a separate reviewer pass.
...
| Codex review (pre-push, Step 6.5) | Fitness for purpose, architectural soundness, maintainability, extensibility, security, established patterns, consistency with the larger codebase. Codex returns structured findings into a protected local artifact. The agent sanitizes public prose and supplies each finding's ADR-029 disposition; the server preserves verdict/finding identity/classification, validates every disposition under the incumbent decision rules, and verifies the exact revision before publishing the canonical issue-thread records. The coding agent dispatches on the returned `next_action` after publication (re-invoke only on `fix_findings_and_reinvoke`; on `fix_findings_then_ask_over_cap_or_proceed`, fix and verify the findings, then ask whether to spend one extra cycle or proceed). Choosing to proceed advances with `accepted_at_cap`; it does not require a clean verdict. | `gc_codex_review_cycle` (`uncommitted=true`, `publication_mode="deferred"`), `gc_get_review_result`, then `gc_publish_review_result` |

**Diff transport and review coverage (issues #1414 and #1632).** `gc_codex_review` owns diff retrieval end to end. It reserves the reviewer prompt wrapper and slice-metadata headroom from `GC_CODEX_REVIEW_MAX_DIFF_BYTES` (default 256 KiB; `0` disables the cap), then splits the diff into bounded inline slices when needed. A diff just below the raw threshold can therefore still be sliced. Slices are not cycles: the per-issue counter, marker family, and cap are unchanged regardless of slice count. Coverage is validated before the first GitHub write; incomplete coverage consumes no cycle. Deferred results expose closed local failure causes, never raw parser or engine output, so an agent can diagnose repo-owned failures before escalating an external dependency.

**Untracked files and the review consent boundary (issue #1414).** Untracked files are the one review input a developer never selected: they are simply present in the working directory, and the branch under review controls `.gitignore`. Narrowing an ignore rule exposes a developer's local `.pgpass` or `.dockercfg`, and no filename deny-list or content heuristic can authorize sending unselected working-tree content to a model provider. `gc_codex_review` therefore never transmits untracked bodies: an `uncommitted=true` review covers tracked committed, staged, and unstaged changes against the merge base with the requested base branch, the prompt says exactly that, the reviewer-visible manifest carries a count of unreviewed untracked paths, and the caller receives the path list off-prompt in `review_coverage.unreviewed_untracked_paths`. Staging is the explicit consent boundary, and Step 6.5 stages with `git add -A` before review, so genuinely new work is still reviewed as staged content.

**Async execution (issues #937, #943, #1473, #1632, and #1669).** Codex review, architecture preflight, repository publish commands, and CI/Sonar watchers can run longer than one MCP request. Wait on one with `gc_codex_job action="await"`, which holds the request server-side until the job is terminal and returns the same envelope `action="poll"` would: waiting costs one call rather than one full-context model turn per tick. The optional `wait_seconds` bounds only that request (default 1500, max 1800); its expiry returns the ordinary running envelope while the job continues, so a longer job is resumed by awaiting again. `action="poll"` remains the immediate observability operation, and awaiting grants no cancellation and changes no retention, idempotency, or job deadline. The public `gc_codex_review_cycle` wrapper is async-only and requires a bounded `idempotency_key`: identical retained starts reuse one running or terminal job, changed input conflicts, and distinct keys are single-flight per canonical repository and issue. In deferred mode the terminal envelope carries a durable `review_handle`, so process-local job expiry does not lose the result. Publication has its own trusted, per-stage retry reconciliation and never reruns the reviewer.

All preflight/review stages operate under the same rule: **fix everything, defer nothing.** The Codex review cap defaults to one cycle and is configurable through `workflow.codex_review.pre_push_cap` (bounds `[1, 10]`). The cap bounds discovery passes; it does not require a clean terminal verdict. After fixes and verification, declining an extra cycle advances the workflow. An extra cycle requires `override_cap=true` plus `override_reason=<authorization quote>`.
...
**Deal with what the run surfaces; do not walk past it (issue #1526).** "Defer nothing" covers the problems an `/implement` run surfaces *incidentally*, not only its review findings: a red CI gate, a flaky test, a scanner finding on a pre-existing line the diff pulled into the new-code leak period, or a latent bug in an adjacent path. Each is dealt with, never routed around. Prohibited routes to green: re-running CI to get past a failure without diagnosing it, reverting or un-touching a change purely to move a finding out of scope, marking a real finding a false positive, or leaving it for later. `/implement` either fixes a straightforward in-scope problem or creates and delivers a tracked issue and PR for a genuinely separate concern. `/quickfix` deliberately has a narrower boundary (ADR-100): it fixes defects caused by or directly blocking its change and reports unrelated concerns with evidence, but does not recursively create another issue, PR, or implementation run. A confirmed-flaky test is made deterministic when it directly blocks the active change; a finding is a false positive only when it is factually wrong, with a rationale that says why.
...
deployment stack they guarded. Broad verification runs in CI. Developers may
run `make mcp-test` or `make policy` explicitly for diagnosis, but neither is a
mandatory local workflow phase.

Inside `/implement` and `/quickfix` an agent does not run `pre-commit` by hand
(issue #899). Targeted tests give edit feedback at Step 5, and the `publish` action runs the one mandatory
pre-publish boundary, `workflow.precommit_command`, before it commits. That
invocation is explicit because hook installation is per clone and cannot be
assumed. The mechanical commit then disables Git hook dispatch, preventing an
installed hook from repeating the same checks on the unchanged staged snapshot.
The agent also leaves committing to `publish`,
because a local commit made earlier skips its sensitive-path screening and
commit-message validation. The `implement-manual-precommit-instruction` policy
check fails when `/implement` or `/quickfix` prose names a `pre-commit run`
invocation outside Step 7.
...
security-sensitive changes, or evidence of wider risk. CI owns repository-wide
completion and policy suites. Step 7 owns the single mandatory pre-publish
pre-commit boundary; required hosted checks block readiness on the current SHA.
...
That invocation checkout is the starting worktree and the mutation boundary
for the run and every delegated step. Agents and delegated agents MUST NOT make
repository changes outside the starting worktree without explicit user
authorization naming the other repository or worktree. Read-only inspection
outside the starting worktree remains allowed. `/integrate` is separately
invoked and its authorization is bounded to the isolated worktrees and
operations documented by that lane.
...
non_verdict_retry_limit` sets how many additional attempts follow the first
(bounds `[0, 2]`, default 1).
...
In deferred mode, non-verdict attempts write nothing to GitHub. After the
bounded re-attempts are exhausted, a separate retained `non_verdict` artifact
carries only closed failure classes and attempt ordinals; explicit publication
opens and escalates the `station_observation` obligation, one per issue,
station, and logical cycle, without consuming a review cycle. A later
attempt that renders a verdict resolves it
...
When the bounded re-attempts are spent and the failure is published, the obligation stays open and escalates
...
## Fast local hooks (issues #1628/#1629)

The default publish command is `pre-commit run --hook-stage pre-commit`, which
checks staged files. Ground Control assigns file hygiene, syntax checks, and
secret scanning to the commit stage only. Push does not repeat them or run
repository policy. Required CI owns full-file scanning, tests, policy, prose
lint, and PR-body validation. Custom repositories should keep broad suites out
of their hook configuration; no tool guesses equivalence between shell commands.
