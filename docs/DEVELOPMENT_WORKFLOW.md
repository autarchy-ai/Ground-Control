...
- **Exact-input verification reuse (#1497, #1626).** When `workflow.verification.toolchain_fingerprint_command` is configured, the `verify` action checks for a trusted content-addressed **verification attestation** before running completion or policy. A hit for the exact tree, freshly resolved base commit, requirement context, command/config digest, and repo-supplied toolchain digest returns reuse immediately. A miss runs the gates and posts the attestation. Base synchronization applies the same decision on already-current trees and committed-merge recovery; a changed merged tree receives fresh verification. Within one MCP process, successful phases are also retained by that same content address, so retrying a failed policy gate reuses completion while every bound input is unchanged. Process restart, missing or untrusted evidence, input drift, or fingerprint failure invalidates reuse. Every result reports `verification_decision`, `verification_reason`, and `broad_gates_executed`; per-gate `timings` mark reused phases. The configured pre-commit boundary still runs once per publish, CI/SonarCloud remain independent remote authorities, and all reuse fails closed. This repository fingerprints Node, Python, dependency locks, hook/prose configuration, and the fingerprint implementation through `tools/verification-fingerprint.mjs`. Design record: `architecture/notes/tiered-publish-verification-preflight.md`.
...
(issue #899). Targeted tests give edit feedback at Step 5, the completion and
policy suites run at Step 6, and the `publish` action runs the one mandatory
