...
| Durable-record MCP tools | `gc_post_decision_record` (Step 6.5 cycle decisions), `gc_post_final_report` (the report substep invoked through the shared finalizer), and `gc_render_pr_body` (Step 9 PR body) replace agent free-prose with deterministic structured-input renderers. `gc_render_pr_body`'s evidence envelope is repo-neutral (#1199): semantic, stack-agnostic attestations, never configured command strings. All three filter sensitive content, post under a structured marker family, and reject `decision: "defer"` server-side. `gc_post_final_report` also requires `/implement` callers to pass `plain_english_outcome`, which renders an Outcome section before the structured evidence. `gc_render_pr_body` takes `lane` and `pre_push_reviews` (issue #1551) to decide which pre-push review attestation the Ground Control Checks section carries: `/implement` uses its configured review contract, while `/quickfix` records whether its one optional Codex cycle ran. Every body carries an accurate attestation. | Always available; shared composites call the mutation tools at their owned boundaries |
