...
| Why did a repeated verification or base-sync retry skip the repository-wide gates? | [`docs/DEVELOPMENT_WORKFLOW.md`](DEVELOPMENT_WORKFLOW.md) and [ADR-021](../architecture/adrs/021-gated-agentic-development-loop.md) - tool-owned evidence is reusable only when repository identity, content tree, base SHA, commands, and toolchain fingerprint remain exact; otherwise the gates run again |
| A PR with `Closes #n` merged but the issue is still open. What closes it? | [`skills/quickfix/SKILL.md`](../skills/quickfix/SKILL.md) Step Q20 and [`skills/implement/steps/step-20-close-issue-on-merge.md`](../skills/implement/steps/step-20-close-issue-on-merge.md) - GitHub honors `Closes #n` only on a default-branch merge, so `gc_close_issue_after_merge` closes the issue after an integration-branch merge |
