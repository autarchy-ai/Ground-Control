...
| `gc_release_identity` | gate/support | Allocates versioned artifact-release identities only through create-only references under `refs/gc/release-identities/<family>/`, against the base head and the family definition on that commit. A malformed or incomplete log, a path collision, or an unconfirmed write refuses; identities are never reissued. It edits no evidence file and changes no other gate. |
